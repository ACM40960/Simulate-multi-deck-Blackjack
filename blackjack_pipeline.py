...
# -*- coding: utf-8 -*-
"""
Blackjack pipeline: dataset generation + analysis + simulation.
Now includes a full Basic Strategy policy (pairs/soft/hard + splits + doubles).

Subcommands:
  dataset  -> generate decision-state CSV (with rollout EVs)
  analyze  -> metrics + EV-by-upcard + heatmaps + hit-threshold charts
  simulate -> full-round Monte Carlo by decks using 'naive' or 'basic' policy

Examples
--------
# 1) Generate dataset (H17) and write CSV
python blackjack_pipeline.py dataset --rows 100000 --out data/blackjack_games.csv

# 2) Analyze CSV -> metrics + plots + threshold
python blackjack_pipeline.py analyze --csv data/blackjack_games.csv --outdir outputs

# 3) Simulate full rounds with FULL BASIC STRATEGY (default), 1 & 2 decks
python blackjack_pipeline.py simulate --policy basic --decks 1 2 --n-games 200000 --replicates 5 --outdir outputs
"""
...
# =============================================================================
# Shared card utils (infinite-deck draws used by dataset generator & dealer)
# =============================================================================
RANKS = [2,3,4,5,6,7,8,9,10,'A']
WEIGHTS = [1,1,1,1,1,1,1,1,4,1]   # 10/J/Q/K collapsed to 10 with weight 4
...
# =============================================================================
# 1) DATASET GENERATION  (R-like, plus rollout EVs)
# =============================================================================
...
# =============================================================================
# 2) ANALYSIS (metrics + heatmaps + EV-by-upcard + hit-threshold)
# =============================================================================
...
# =============================================================================
# 3) FULL-GAME MONTE CARLO (now supports BASIC STRATEGY)
# =============================================================================
@dataclass
class Rules:
    ...
    allow_double_any: bool = True    # double on any two (common)
    surrender: bool = False          # not implemented in policy below
...
class BasicStrategy:
    """
    Table-driven basic strategy (typical H17 + DAS).
    ...
    def decide(self, hand: Hand, dealer_up, can_double: bool, can_split: bool, after_split: bool) -> str:
        ...
        if t == 11:
            return 'D' if can_double else 'H'  # vs A is also double in H17
        if t == 10:
            return 'D' if (can_double and up in [2,3,4,5,6,7,8,9]) else 'H'
        if t == 9:
            return 'D' if (can_double and up in [3,4,5,6]) else 'H'
...
def play_player_basic(initial: Hand, dealer_up, shoe: Shoe, rules: Rules) -> List[Tuple[int, int, bool]]:
    """
    Plays the player's hand(s) under basic strategy.
    Returns list of tuples: (final_total, bet_units, is_bust).
    """
    ...
    while stack:
        ...
        while True:
            ...
            can_double = (len(hand.cards) == 2) and rules.allow_double_any and (not after_split or rules.das)
            ...
            if action == 'P' and can_split:
                ...
                ace_split = (rank == 'A')
                # push the second; continue with the first (depth-first)
                ...
                # if split aces with no hits, loop will immediately resolve next iteration
                continue
